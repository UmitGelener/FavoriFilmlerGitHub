                    ...
                    if (index < 2) {
                        $.getJSON("https://www.omdbapi.com/?", { apikey: "fd161998", i: value.imdbID }, function (movieData) {
                            if (movieData) {
                                ...
                                results += '<div>Stars: ' + movieData.Actors.split(",").slice(0, 3) + '</div>'
                                results += '</div>'
                                results += '<div>Imdb Rating: ' + movieData.imdbRating + '</div>'


                                results += '<div>'
                                results += ' <form action="/Index/favori" class="login" method="post">'
                                results += '<input type="text" name="title" value="' + movieData.Title + '"  style="display:none;" >'
                                results += '<input type="text" name="year" value="' + movieData.Year + '"  style="display:none;" >'
                                results += '<input type="text" name="imdb_rating" value="' + movieData.imdbRating + '"  style="display:none;" >'
                                results += '<input type="text" name="director" value="' + movieData.Director + '"  style="display:none;" >'
                                results += '<input type ="submit" value ="Favoriye Ekle">'
                                results += '</form>'
                                results += '</div>'
